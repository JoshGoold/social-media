                ...
                <div className='p-2 flex flex-col gap-3'>
                    {props.userData.posts.map((post, index) => (
                        <div className="border-t-gray-300 shadow-md rounded-md p-3 bg-white border" id={post._id} key={index}>
                            <button className='hover:scale-110 rounded-full ' onClick={()=>deletePost(String(post._id))} title='Delete Post'>🔴</button>
                            <img src={`http://localhost:3000${post.postImg}`} alt="Post" />
                            <p>{post.postContent}</p>
                            <small className='text-sm text-gray-400 font-thin'>{curMonth}/{post.createdAt}</small>
