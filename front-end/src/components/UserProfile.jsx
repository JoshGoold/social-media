...
const UserProfile = () => {
    ...
    function checkFollowing(){
        try {
            const isFollowing = userData?.followers.find(follower=> follower.id === user.id)
  ...
  return (
    <div className='p-3 overflow-scroll  h-screen'>
    <button className='p-2 bg-blue-500 text-white font-thin rounded-lg' onClick={()=> nav(`/dashboard/${user.username}`)}>Back</button>
        
        <div className="p-2 flex w-full  flex-col justify-center items-center">
            <div className="">
                <UserProfileHead userData={userData}/>
                <div className="flex flex-col">
                <Follow followState={following} handleUserProfile={handleUserProfile} username={userData.username}/>
                ...
                <div className="w-full  ">
                <UserPost userData={userData}/></div>
                </div>
            </div>
